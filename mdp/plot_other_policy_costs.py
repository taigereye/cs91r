...
def main(argv):
    try:
        ...
        print('usage: plot_other_policy_costs.py -m <modelversion> -p <paramsfile> -a <policyfile> -v <techstage>')
        sys.exit(1)

    version = str(opts[0][1])
    if int(version) < 1:
        print("error: plot_other_policy_costs only supported for MDP v2 or higher.")
        sys.exit(2)

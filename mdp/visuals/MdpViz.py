...
class MdpDataGatherer():
    def __init__(self, mdp_model, n_iter, t_range, ci_type="ABS"):
    ...
    def _calc_data_bounds(self, data_all, axis=0, n_dev=2):
        if self.ci_type == "ABS":
            return self._calc_data_bounds_abs(data_all, axis)
        elif self.ci_type == "STD":
            return self._calc_data_bounds_std(data_all, axis, n_dev)
        else:

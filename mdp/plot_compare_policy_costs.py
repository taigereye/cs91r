...
def main(argv):
    ...
    if int(args.version) < 2:
        print("error: plot_compare_policy_costs only supported for MDP v2 or higher.")
        sys.exit(1)

    if args.policies is not None and len(args.policies) not in (1, 2):
        print("error: plot_compare_policy_costs takes 2 arbitrary policies or 1 arbitrary and the optimal policy.")
        sys.exit(1)
